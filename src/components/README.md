...
- `ui/` — small generic pieces reused across more than one place
  (`EmptyState`, used by both the Projects and Settings areas;
  `StatusPill`, used by `Header` for backend connectivity and by
  `SettingsPage` for local storage status; `Dialog`, the modal shell used
  by both the create- and delete-project dialogs).
